const marketsCardsWrapper = document.querySelector('.markets-cards__wrapper')
let marketsCardsHTML = ''

let showModalHandlerForMarkets = null



const marketsCardHTML = (obj, id) => {
  ...
  showModalHandlerForMarkets = (target, idx) => {
    openedMyCardsObj = {
      ...marketsCards[idx],
      target,
      id: idx,
...
const generateMarketsCardsHTML = () => {
  marketsCardsHTML = ''
  marketsCards.forEach((card, id) => {
    marketsCardsHTML += marketsCardHTML(card, id)
  })

  marketsCardsWrapper.innerHTML = marketsCardsHTML
  marketsCardsWrapper.childNodes.forEach((childNode, idx) => {
    childNode.onclick = () => showModalHandlerForMarkets(childNode, idx)
  })
}

const saveMarkets = () => {
  localStorage.setItem('marketsCards', JSON.stringify(marketsCards))
}
